...
install_all <- function(){
  ...
  BiocManager::install(c("npGSEA", "GSEABase","limma","org.Hs.eg.db","GenomicRanges","DESeq2"))
...
# select rows by list of values in a given column
## inputs: df, name_col = a column of the df, list_values = a list of values to search for
select_rows_by_col_vals <- function(df, name_col, list_values){
  filtered_df <- df %>% 
    filter(grepl(paste(list_values, collapse="|"), name_col))
}
...
# exactly like dplyr::group_split except it returns a named list of the resultant dataframes. See romainfrancois at https://github.com/tidyverse/dplyr/issues/4223
